...
namespace Utilities.Events
{
    public class EventBus<TBaseEvent> : IDisposable
    {
        ...
        protected event Action<EventBus<TBaseEvent>> DisposeEvent;
        ...
        /// <typeparam name="TEvent">The type of event to raise.</typeparam>
        /// <returns>
        ///     If the event was raised immediately, returns true if the event was consumed with <see cref="ConsumeCurrentEvent"/>.
        /// </returns>
        ...
        public virtual void SubscribeTo<TEvent>(EventHandler<TEvent> handler, float priority = 0) where TEvent : TBaseEvent
        {
            var listeners = EventListeners<TEvent>.GetListeners(this);
        ...
        {
            private static readonly Dictionary<EventBus<TBaseEvent>, EventListeners<TEvent>>
                Listeners = new Dictionary<EventBus<TBaseEvent>, EventListeners<TEvent>>();
